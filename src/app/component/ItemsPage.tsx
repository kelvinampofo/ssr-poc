...
export default function ItemsPage() {
  ...
  const { items, loading, createItem, deleteItem, updateItem } = usePosts();
  ...
  const startEdit = (item: { id: number; title: string; content: string }) => {
  ...
  const handleUpdate = async (id: number) => {
    await updateItem(id, editTitle, editContent);
    setEditingId(null);
    setEditTitle("");
    setEditContent("");
  };
    ...
    <div className="items-container">
      {loading ? (
        ...
        <>
          ...
          {items.length === 0 ? (
            <p className="empty-message">Looks a little empty here.</p>
          ) : (
